...
const submit = () => {
    form.post(route("proyectos.productos.store", props.proyecto.id));
};

const productosOpciones = computed(() => {
    if (!props.productosDisponibles) return [];
...
<template>
    <Head title="Agregar Producto al Proyecto" />
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                :class="form.errors.producto_id ? 'select-error' : ''"
                                style="
                            ...
                            >
                                ...
                                <option
                                    v-for="producto in productosOpciones"
                                    :key="producto.id"
                                    :value="producto.id"
                                >
                                    {{ producto.nombre }} - {{ producto.tipo }} (Stock: {{ producto.stock }}) - {{ formatCurrency(producto.precio_unitario) }}
                                </option>
                            </select>
                            <div v-if="form.errors.producto_id" class="text-error text-sm mt-1">
                        ...
                        <div>
                            ...
                            <p class="text-xs mt-1" style="color: var(--theme-text-secondary);">
                                La cantidad será restada del stock general del producto
                            </p>
                        </div>
                        ...
                        <div class="flex gap-3 pt-6 border-t" style="border-color: var(--theme-border)">
                            ...
                            >
                                ...
                                {{
                                    form.processing ? "Guardando..." : "Agregar Producto"
                                }}
                            </button>
                            <Link
