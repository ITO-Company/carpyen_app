...
const form = useForm({
    user_id: props.tarea.user_id,
    fecha: new Date(props.tarea.fecha).toISOString().split('T')[0],
    hora_inicio: props.tarea.hora_inicio,
    hora_fin: props.tarea.hora_fin,
    descripcion: props.tarea.descripcion,
    estado: props.tarea.estado,
});

const submit = () => {
    form.put(route("tareas.update", props.tarea.id));
};
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select
                                v-model="form.estado"
                                class="select select-bordered w-full"
                                :class="form.errors.estado ? 'select-error' : ''"
                                style="
                                    background-color: var(--theme-bg-primary);
                                    color: var(--theme-text-primary);
                                    border-color: var(--theme-border);
